...
- 图片生成/编辑默认使用 2K 画质（`resolution: 2k` 透传 Grok2API）；`image_media_route` 切到 `Web` 时自动锁定 `1k`（官方 Grok Web 图片接口仅支持 resolution=1k）
...
- 图片对话链路已放开工具调用（Grok 对话生图依赖 `imagine` 工具，由 Grok2API 后端执行后返回媒体链接）
