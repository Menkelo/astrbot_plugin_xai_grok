...
- 官方 xAI 视频接口会把尺寸映射为 `aspect_ratio`；旧 Grok2API 链路会透传为 `video_config.size`
- 可写 `480p`、`720p`、`1080p` 指定官方 xAI 视频接口的 `resolution`
- 使用官方 `grok-imagine-video*` 视频接口时，时长支持 `1-15s`，例如 `15s` 会精确透传为 `duration=15`
- `grok-imagine-video-1.5-preview` 仅用于图生视频；无参考图时请配置文生视频槽位为 `grok-imagine-video`
...
- Video Generation 接口：`/v1/videos/generations`
  - 优先用于官方 `grok-imagine-video*` 视频模型
  - 支持 `duration` 1-15 秒、`aspect_ratio`、`resolution`，图生视频通过 `image.url` 传参考图
  - 若后端不支持该端点，会回退到旧 `chat/completions` 视频链路
...
官方 xAI 视频接口看日志是否出现 `aspect_ratio=16:9` 这类参数；插件会把支持尺寸自动映射为比例。
